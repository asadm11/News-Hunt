...
const News = (props) => {
  ...
  const updateNews = async () => {
    ...
    // const url = `/api?country=${props.country}&category=${props.category}&apiKey=4d6cc77a4bef42398c99c54dedd9e070&page=${page}&pageSize=${props.pageSize}`;
    const url = `/api?country=${props.country}&category=${props.category}&apiKey=${props.apikey}&page=${page}&pageSize=${props.pageSize}`;
    setLoading(true);
  ...
  const fetchMoreData = async () => {
    const url = `/api?country=${props.country}&category=${props.category}&apiKey=${props.apikey}&page=${page + 1}&pageSize=${props.pageSize}`;
    setPage(page + 1);
    // this.setState({ loading: true });
    let data = await fetch(url);
    let parsedData = await data.json();
  ...
  return (
    <>
      <h2 style={{ margin: "100px 0px 10px", textAlign: "center" }}>
              ...
              return (
                <div className="col-md-4" key={element.url}>
                  <NewsItem
                    ...
                    urlImage={
                      !element.urlToImage
                        ? "https://image.shutterstock.com/image-vector/news-concept-thin-line-pixel-260nw-573975388.jpg"
                        : element.urlToImage
                    }
                    urlNews={element.url}
